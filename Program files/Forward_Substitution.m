...
% Here, we are using Forward substitution which will give us a new coefficient 
% matrix which will be in a form of Lower triangular matrix, along with new
...
for j = 1:n
    for i = n-j:-1:1
        ...
        Y(i,1) = Y(i,1) - (A(i,n-j+1)/A(n-j+1,n-j+1)).*Y(n-j+1,1); % Updating i^th element in Y
        A(i,:) = A(i,:) - (A(i,n-j+1)/A(n-j+1,n-j+1)).*A(n-j+1,:); % Updating i^th row in A
    end
end
